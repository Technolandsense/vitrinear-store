...
export default function AdminNav() {
      ...
      <div style={{ background: '#0F1923', padding: '0 20px', display: 'flex', alignItems: 'center', height: 56, gap: 4 }}>
        ...
        {links.map(l => (
          <button key={l.path} onClick={() => router.push(l.path)}
            style={{ padding: '8px 14px', borderRadius: 8, background: router.pathname === l.path ? 'rgba(255,255,255,.12)' : 'transparent', color: router.pathname === l.path ? '#fff' : 'rgba(255,255,255,.6)', fontWeight: 600, fontSize: 12 }}>
            {l.label}
          </button>
        ))}
        <div style={{ flex: 1 }} />
  ...
  return (
    <div style={{ background: '#0F1923', padding: '0 20px', display: 'flex', alignItems: 'center', height: 56, gap: 4 }}>
      <span style={{ color: '#fff', fontWeight: 800, fontFamily: "'Syne',sans-serif", fontSize: 16, marginRight: 20 }}>
        {STORE_SLUG}<span style={{ color: 'rgba(255,255,255,.3)', fontSize: 10, marginLeft: 8 }}>ADMIN</span>
      </span>
      {links.map(l => (
        <button key={l.path} onClick={() => router.push(l.path)}
          style={{ padding: '8px 14px', borderRadius: 8, background: router.pathname === l.path ? 'rgba(255,255,255,.12)' : 'transparent', color: router.pathname === l.path ? '#fff' : 'rgba(255,255,255,.6)', fontWeight: 600, fontSize: 12 }}>
